 ...
 *  2.  Players & listeners stored in CopyOnWriteArrayList → safe iteration under mutation.
 *  3.  Single ReentrantLock guards all mutable state that must change atomically.
 *  4.  Deck is final; shuffle() is delegated to callers – deal() no longer triggers an implicit shuffle.
 *  5.  Removed Android‑specific Log calls from model layer (use listener hooks).
 *  6.  Public APIs are fail‑fast (e.g. adding players after the game has started throws an exception).
 *  7.  Minimized shared‑mutable access via volatile fields and confined collection mutations under lock.
 */
public final class Game {

    /* -----------------------------  state & constants  ----------------------------- */

    public enum State { WAITING, DEALING, PLAYING, GAME_OVER }  // 等待发牌，发牌中，游戏进行中，游戏结束
    ...
    public boolean playSelected() {
        ...
        try {
            ...
            if (sel.isEmpty()) return false;  // 如果选中的牌为空，则返回 false

            CardPattern pat = validator.validate(sel);  // 验证牌型
            if (pat.getPatternType() == CardPattern.INVALID) return false;  // 如果牌型无效，则返回 false
            ...
            if (!isFreeTurn() && !canBeat(pat)) return false;  
    ...
    private void ensurePlayerCount() {
        if (!autoFillBots) return;  // 如果不需要自动填充机器人，则返回    
        while (players.size() < 2) {  // 如果玩家数量小于2，则添加机器人
            players.add(new AIPlayer("AI" + players.size(), new AdvancedAIStrategy()));  // 添加机器人
        }
        while (players.size() > 4) players.remove(players.size() - 1);  // 如果玩家数量大于4，则移除最后一个玩家
    }
    // 发牌
    private void deal() {
        state = State.DEALING;  // 设置状态为发牌中
        long seed = players.stream().map(Player::getName).mapToLong(String::hashCode).sum();  // 生成种子
        deck.shuffle(seed);  // 洗牌
        List<List<Card>> hands = deck.dealCards(players.size());  // 发牌
        for (int i = 0; i < players.size(); i++) players.get(i).setHand(hands.get(i));  // 设置玩家手牌
    }
    ...
    private int startIdx() {
        for (int i = 0; i < players.size(); i++)
            if (playerHas(players.get(i), Card.DIAMOND, Card.THREE)) return i;  // 如果玩家有方片3，则返回该玩家索引
        return 0;  // 否则，返回0
    }
    ...
    private boolean canBeat(CardPattern next) {
        if (next == null) return false;  // 如果下一轮牌型为空，则返回false
        if (lastPattern == null) return true;  // 如果上一轮牌型为空，则返回true
...
// 执行出牌
    private void executePlay(Player p, List<Card> cards, CardPattern pat) {
        ...
        if (p.getHand().isEmpty()) finish(p);  // 如果玩家手牌为空，则结束游戏
        else next();  // 否则，下一轮
}
    ...
    private void next() { currentIdx = (currentIdx + 1) % players.size(); }  // 下一轮

    private Player current() { return players.get(currentIdx); }  // 获取当前玩家  

    private boolean isFirstTurn() { return lastIdx == -1; }  // 是否是第一轮
    private boolean isFreeTurn()  { return lastPattern == null || passCount >= players.size() - 1; }  // 是否是自由出牌
    ...
    private void fire(java.util.function.Consumer<GameStateListener> fn) {
        for (GameStateListener l : listeners) {
            try { fn.accept(l); } catch (Exception ignored) {}
        }
    }
    ...
    public CardPattern getLastPattern()  { return lastPattern; }

    public void setAutoFillBots(boolean b) { autoFillBots = b; }  // 设置是否自动填充机器人
