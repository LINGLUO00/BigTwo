...
public class NetworkController implements NetworkManager.ConnectionListener, NetworkManager.MessageListener {
    ...
    public void setGameController(GameController gameController) {
        ...
        if (isHostFlag && gameController!=null && !pendingJoins.isEmpty()){
            for(String n: pendingJoins){ gameController.addRemotePlayer(n);} 
            pendingJoins.clear();
        }
    }
    ...
    public void initialize(NetworkManager.ConnectionType type) {
        executors.io().execute(() -> {
            try {
                ...
                mainHandler.post(() -> currentView.showMessage("网络连接已初始化"));
            } catch (Exception e) {
                mainHandler.post(() -> currentView.showMessage("初始化网络失败: " + e.getMessage()));
                Log.e(TAG, "初始化网络失败", e);
            }
        });
    }
    ...
    public void startDiscovery() {
        executors.io().execute(() -> {
            try {
                ...
                mainHandler.post(() -> currentView.showMessage("开始扫描设备..."));
            } catch (Exception e) {
                mainHandler.post(() -> currentView.showMessage("搜索设备失败: " + e.getMessage()));
                Log.e(TAG, "搜索设备失败", e);
            }
        });
    }
    ...
    public void stopDiscovery() {
        executors.io().execute(() -> {
            try {
                ...
                mainHandler.post(() -> currentView.showMessage("停止搜索设备"));
            } catch (Exception e) {
                Log.e(TAG, "停止搜索时出错", e);
            }
        });
    ...
    public boolean createRoom(String roomName) {
        try {
            ...
            if (success) isHostFlag = true;
            mainHandler.post(() -> currentView.showMessage(success ? "创建房间成功" : "创建房间失败"));
            return success;
        } catch (Exception e) {
            mainHandler.post(() -> currentView.showMessage("创建房间出错: " + e.getMessage()));
            Log.e(TAG, "创建房间出错", e);
            return false;
        }
    }
        ...
        executors.main().execute(() -> {
            switch (type) {
                ...
                case GAME_START:
                    ...
                    break;
                case DEAL_CARDS:
                    currentView.showMessage("发牌数据收到");
                    if(gameController!=null){
                ...
                case PLAY_BROADCAST: // 所有端都应用
                    currentView.showMessage("玩家出牌: " + data);
                    if (gameController != null) {
                        applyPlayCards(data);
                    }
                    break;
    ...
    /** 供外部判断当前是否为房主 */
    public boolean isHost() { return isHostFlag; }
    ...
    private void applyPlayCards(String data){
        try{
            ...
            currentView.showMessage("DBG recv play body="+body);
            ...
            for(int i=0;i<g.getPlayers().size();i++){
                currentView.showMessage("DBG check player="+g.getPlayers().get(i).getName());
                ...
                if(p.getName().equalsIgnoreCase(name)){
                    ...
                    for(String tk: cardTokens){
                        if(tk.trim().isEmpty()) continue;
                        String[] sr = tk.split("\\.");
                        int suit=Integer.parseInt(sr[0]);
                        int rank=Integer.parseInt(sr[1]);
                        models.Card newCard = new models.Card(suit,rank);
                        parsed.add(newCard);
                        for(models.Card c: p.getHand()) if(c.getSuit()==suit && c.getRank()==rank){remove.add(c);break;}
                    }
            ...
            currentView.showMessage("DBG playedIdx="+playedIdx+" nextName="+nextName);
            // 推进 currentIdx 到下一位玩家（仅客户端侧估算）
            if(nextName!=null){
                try{
                    for(int i=0;i<g.getPlayers().size();i++) if(g.getPlayers().get(i).getName().equalsIgnoreCase(nextName)){
                        java.lang.reflect.Field idxF = g.getClass().getDeclaredField("currentIdx");
                        idxF.setAccessible(true);
                        idxF.setInt(g, i);
                        currentView.showMessage("DBG reflect set currentIdx="+i);
                        break;
                    }
                    if(playedIdx>=0){
                        ...
                        currentView.showMessage("DBG set current="+nextName+" last="+playedIdx);
                    }
                }catch(Exception e){ currentView.showMessage("DBG reflectErr="+e.getMessage()); }
            } else if(playedIdx>=0){
                try{
                    ...
                    lastF.setInt(g, playedIdx);
                    currentView.showMessage("DBG set current="+nextName+" last="+playedIdx);
                }catch(Exception e){ currentView.showMessage("DBG reflectErr2="+e.getMessage()); }
            }

            try {
                currentView.showMessage("DBG before updateGS cur="+g.getCurrentPlayer().getName());
            } catch(Exception e) {currentView.showMessage("DBG gsErr="+e.getMessage());}
            gameController.updateGameState(g);
            try {
                currentView.showMessage("DBG after updateGS uiCur="+g.getCurrentPlayer().getName());
            }catch(Exception e){currentView.showMessage("DBG uiErr="+e.getMessage());}
            ...
            try {
                ...
                pcF.setInt(g, 0);
            } catch(Exception e){ currentView.showMessage("DBG syncPatternErr="+e.getMessage()); }
    ...
    private void handlePlayRequestAsHost(String data){
        ...
        for(models.Player p: g.getPlayers()) if(p.getName().equalsIgnoreCase(reqPlayer)){ target = p; break; }
        ...
        currentView.showMessage("REQ:"+reqPlayer+" cards="+cardPart);

        boolean ok = g.playSelected();
        currentView.showMessage("DBG host playSelected ok="+ok);
        if(!ok){
            currentView.showMessage("DBG illegal play, ignored");
            return; // 非法牌，忽略
        }
    ...
    private void handlePassBroadcast(String data) {
        ...
        currentView.showMessage("DBG PASS recv name="+name+" next="+next);
        gameController.handlePassBroadcast(name);
        currentView.showMessage("DBG after handlePassBroadcast cur="+gameController.getGame().getCurrentPlayer().getName());
        if(next!=null){
            Game g = gameController.getGame();
            if(g!=null){
                try{
                    for(int i=0;i<g.getPlayers().size();i++) if(g.getPlayers().get(i).getName().equalsIgnoreCase(next)){
                        java.lang.reflect.Field idxF = g.getClass().getDeclaredField("currentIdx");
                        idxF.setAccessible(true);
                        idxF.setInt(g, i);
                        break;
                    ...
                    gameController.updateGameState(g);
                    currentView.showMessage("DBG after reflect pass cur="+g.getCurrentPlayer().getName());
                }catch(Exception ignored){}
            }
        }
    }
